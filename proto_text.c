...
// 1: incr(ement)
// 2: mult(iply)
static void process_arithmetic_command(conn *c, token_t *tokens, const size_t ntokens, const int operation) { // KIVA KIVA KIVA
    ...
    switch(add_delta(c->thread, key, nkey, operation /* KIVA KIVA KIVA */, delta, temp, NULL)) {
    case OK:
        out_string(c, temp);
        break;
    case NON_NUMERIC:
    ...
    case DELTA_ITEM_NOT_FOUND:
        pthread_mutex_lock(&c->thread->stats.mutex);
        if (operation == 1) { // KIVA KIVA KIVA
            c->thread->stats.incr_misses++;
        } else if (operation == 0) {
            c->thread->stats.decr_misses++;
        } // Could handle multiply here but, no
        pthread_mutex_unlock(&c->thread->stats.mutex);
...
void process_command_ascii(conn *c, char *command) {
    ...
    if (first == 'm' && tokens[COMMAND_TOKEN].length == 2) {
		...
		if (strcmp(tokens[COMMAND_TOKEN].value, "mult") == 0) { // KIVA KIVA KIVA
			WANT_TOKENS(ntokens, 4, 4); // KIVA KIVA KIVA exactly 4 tokens, no "noreply" supported
            process_arithmetic_command(c, tokens, ntokens, 2); // KIVA KIVA KIVA
		} else {
			out_string(c, "ERROR"); // KIVA KIVA KIVA
		}
    } else if (first == 'g') {
