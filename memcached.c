 ...
 * it    item to adjust
 * operation  0 to decrement, 1 to increment, 2 to multiply value
...
enum delta_result_type do_add_delta(LIBEVENT_THREAD *t, const char *key, const size_t nkey,
                                    const int operation /* KIVA KIVA KIVA */, const int64_t delta,
    ...
    if (operation == 1) { // KIVA KIVA KIVA incr
        value += delta;
        //MEMCACHED_COMMAND_INCR(c->sfd, ITEM_key(it), it->nkey, value);
    } else if (operation == 0) { // KIVA KIVA KIVA decr
        if(delta > value) {
            value = 0;
        } else {
            value -= delta;
		...
		value *= delta; // KIVA KIVA KIVA multiply
	}
    ...
    } // KIVA KIVA KIVA Could handle mult but, no
    pthread_mutex_unlock(&t->stats.mutex);
