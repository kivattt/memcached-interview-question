...
static void process_arithmetic_cmd(LIBEVENT_THREAD *t, mcp_parser_t *pr, mc_resp *resp, const int operation) { // KIVA KIVA KIVA
    ...
    switch(add_delta(t, key, nkey, operation, delta, temp, NULL)) {
    case OK:
    ...
    case DELTA_ITEM_NOT_FOUND:
        ...
        if (operation == 1) {
            ...
            t->stats.decr_misses++;
        } // KIVA KIVA KIVA Could handle multiply but, no
        pthread_mutex_unlock(&t->stats.mutex);
...
int mcplib_internal_run(mcp_rcontext_t *rctx) {
    ...
    switch (rq->pr.command) {
        ...
        case CMD_DECR:
            ...
            break;
		case CMD_MULT:
            process_arithmetic_cmd(t, pr, resp, 2);
			break;
        case CMD_DELETE:
